...
namespace Mango {
	...
	namespace Utils
	{
		...
		static void AttachColorTexture(uint32_t id, int samples, GLenum format, uint32_t width, uint32_t height, int index)
		{
			...
			// TODO: validate if multisampled praameters are any different 
		...
		{
			bool multisampled = samples > 1;
			// TODO: should we be using TexImage for multisampl and TexStorage non multisample?
			if (multisampled)
			{
				glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, samples, format, width, height, GL_FALSE);
			}
	...
	void OpenGLFramebuffer::Invalidate()
	{
		...
		{
			MGO_CORE_ASSERT(m_ColorAttachments.size() <= 4);
		...
		MGO_CORE_ASSERT(glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE, "Framebuffer is incomplete!");
	...
	void OpenGLFramebuffer::Resize(uint32_t width, uint32_t height)
	{
		...
		{
			MGO_CORE_WARN("Attempting to resize framebuffer to {0}, {1}", width, height);
			return;
		}
