	...
	public:
		...
		void BeginSession(const std::string& name, const std::string& filepath = "results.json")
		{
			...
			if (m_CurrentSession)
			{
				...
				{
					MGO_CORE_ERROR("Instrumentor::BeginSession('{0}') when session '{1}' already open.", name, m_CurrentSession->Name);
				}
				InternalEndSession();
			}
			...
			else
			{
				if (Log::GetCoreLogger()) // Edge case: BeginSession() might be before Log::Init()
				{
					MGO_CORE_ERROR("Instrumentor could not open results file '{0}'.", filepath);
				}
			}
		}
...
#define MANGO_PROFILE 0
	...
	#define MGO_PROFILE_SCOPE(name) constexpr auto fixedName = ::Mango::InstrumentorUtils::CleanupOutputString(name, "__cdecl ");\
									::Mango::InstrumentationTimer __MGO_TOKEN_EVALUATE__(__timer,__COUNTER__)(fixedName.Data)
